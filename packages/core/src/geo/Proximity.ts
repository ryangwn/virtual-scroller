...
export type TriggerCause = 'INITIAL_POSITION' | 'MOVEMENT' | 'LIST_UPDATE' | null;
...
export const condition = {
    nearTop: (threshold: number) => (listRect: Rectangle, viewportRect: Rectangle) =>
        viewportRect.getTop() - listRect.getTop() <= threshold,
    nearBottom: (threshold: number) => (listRect: Rectangle, viewportRect: Rectangle) =>
        listRect.getBottom() - viewportRect.getBottom() <= threshold,
    nearTopRatio: (ratio: number) => (listRect: Rectangle, viewportRect: Rectangle) => {
    ...
    nearBottomRatio: (ratio: number) => (listRect: Rectangle, viewportRect: Rectangle) => {
        const viewportHeight = viewportRect.getHeight();
        const threshold = viewportHeight * ratio;
...
export class EdgeProximity {
    ...
    handlePositioningUpdate(positioning: Positioning) {
        for (const { zone, state } of this.handlers) {
            const { callback } = zone;
            const { proximity, listLength } = state;

            const newProximity = zone.condition(positioning.getForList(), positioning.getForViewport())
                ? 'INSIDE'
                : 'OUTSIDE';
            ...
            } else if (proximity === 'INSIDE' && newProximity === 'INSIDE' && listLength !== newLength) {

...
export class Virtualizer<TItemElement extends Element> {
    ...
    private _sizesKnown: Map<string, number> = new Map();
    ...
    private _lastUpdateReason: (typeof UpdateReason)[keyof typeof UpdateReason] | undefined;
    ...
    private _finalRenderedItems = memo(
        ...
        (_list: Array<ListItem>, renderedItemStates: Anchor[]) => {
            ...
            return filterMap(renderedItemStates, (state: Anchor) => {
                ...
                return item ? { item, offset: state.offset, visible: state.visible } : undefined;
            });
        },
        ...
        const get = () => {
            ...
            _ro = new ResizeObserver((entries) => {
                for (let i = 0; i < entries.length; i++) {
                    ...
                    this._options.useAnimationFrameWithResizeObserver ? requestAnimationFrame(run) : run();
                }
            });
            return _ro;
        };
    ...
    mount() {
        ...
        const _removeScrollHandler = this._viewport.addScrollListener(this._handleScroll.bind(this));
        ...
        this._edgeProximity = new EdgeProximity([
            ...
            {
                condition: condition.nearTopRatio(this._options.nearStartProximityRatio ?? 0.25),
                callback: (data) => this._options.onNearStart?.(data),
            },
            {
                condition: condition.nearBottomRatio(this._options.nearEndProximityRatio ?? 1.75),
    ...
    private _handleHeightChanged(itemId: string, height: number) {
        if (this._sizesKnown.get(itemId) !== height) {
    ...
    private _updateItemHeight(itemId: string, height: number) {
        ...
        const allRenderedHaveHeights = this._renderedItems.every(
            ({ itemId }) => this._sizesKnown.has(itemId) || this._pendingHeightUpdates.has(itemId),
        );

        if (allRenderedHaveHeights || this._pendingHeightUpdates.size > 50) {
            for (const [id, height] of this._pendingHeightUpdates) {
                this._sizesKnown.set(id, height);
            }
    ...
    private _measureElement(node: TItemElement, entry: ResizeObserverEntry | undefined) {
        ...
        const heightValue = this._sizesKnown.get(itemId);
        const oldHeight = heightValue !== undefined ? Math.floor(heightValue) : null;
    ...
    private _getAnchorItemCandidates() {
        ...
        const potentialAnchors = currentlyRendered.filter(
            ({ item }) => item.canBeAnchor && this._sizesKnown.has(item.id),
        );
    ...
    private _measureHeights() {
        for (const [node, itemId] of this._cells.entries()) {
            ...
            const currentHeight = this._sizesKnown.get(itemId);

            if (measuredHeight !== currentHeight) {
                this._sizesKnown.set(itemId, measuredHeight);
    ...
    private _updateRenderedItems(anchor: Anchor, relativeViewportRect: Rectangle) {
        const { onChange } = this._options;
        const { allItemsWithPositions, newRenderedItems, slice, arePreferredItemsRendered } = this._getRenderCandidates(
            anchor,
        ...
        const listHeight = this._getHeightBetweenItems(firstItemOverall, lastItemOverall);
    ...
    private _shouldTriggerOnChange(sliceChanged: boolean): boolean {
        return sliceChanged || this._lastUpdateReason === UpdateReason.HEIGHT_CHANGE;
    }

    private _getRenderCandidates(anchor: Anchor, relativeViewportRect: Rectangle) {
        const { minimumOffscreenToViewportRatio, preferredOffscreenToViewportRatio } = this._options;

        // Calculate viewport boundaries with added buffer ratios
        const minBufferRect = this._getBufferedViewport(relativeViewportRect, minimumOffscreenToViewportRatio);
        const preferredBufferRect = this._getBufferedViewport(relativeViewportRect, preferredOffscreenToViewportRatio);
        ...
        const targetBufferRect = usePreferredBuffer ? preferredBufferRect : minBufferRect;
        ...
        const candidateSlice = this._getSliceForCandidates(candidateItems, allItemsWithPositions);

        // 4. Adjust the slice based on the previous slice to avoid large jumps
        const newSlice = this._adjustSlice(this._slice || { start: 0, end: 0 }, candidateSlice, usePreferredBuffer);

        // 5. Extract the final items to be rendered based on the adjusted slice
        const newRenderedItems = allItemsWithPositions.slice(newSlice.start, newSlice.end);
    ...
    private _getBufferedViewport(viewportRect: Rectangle, ratio: number) {
        ...
        return new Rectangle(viewportRect.getTop() - bufferHeight, viewportRect.getHeight() + 2 * bufferHeight);
    }

    private _getDistanceFromTop(targetItemId: string) {
        const targetIndex = this._list.findIndex((item) => item.id === targetItemId);
        if (targetIndex <= 0) return 0;

        let sum = 0;
        for (let i = 0; i < targetIndex; i++) {
            sum += this._getHeight(this._list[i]);
        }
        return sum;
    }
    ...
    private _getItemsWithPositions(anchor: Anchor) {
        ...
        for (let i = 0; i < this._list.length; i++) {
            ...
            allItemsPositions.push(
                new Anchor(item.id, currentOffset, this._sizesKnown.has(item.id), item.canBeAnchor, height),
            );
    ...
    private _getSliceForCandidates(candidates: Anchor[], allItems: Anchor[]) {
        const firstCandidate = first(candidates);
    ...
    private _getHeightForItemId(itemId: string, itemType = '') {
        ...
        const cachedHeight = this._sizesKnown.get(itemId);
    ...
    private _adjustSlice(prevSlice: Slice, nextSlice: Slice, usePreferredBuffer: boolean) {
        ...
        const expansion = Math.max(prevSlice.start - nextSlice.start, nextSlice.end - prevSlice.end, 0);
        return {
    ...
    private _getIsHeightsReady(renderedItems: Anchor[]) {
        ...
        return renderedItems.every(({ itemId }) => this._sizesKnown.has(itemId));
    }

    private _getHeightBetweenItems(firstItemState: Anchor | undefined, lastItemState: Anchor | undefined) {
    ...
    private _updatePositioning({
        ...
        const positioning = new Positioning({
            ...
            renderedItems: renderedItems.map((item) => ({
                id: item.itemId,
                rectangle: new Rectangle(item.offset, this._getHeightForItemId(item.itemId)),
            })),
        });
    ...
    private _scheduleCriticalUpdate = () => requestAnimationFrame(() => this._update());
    private _scheduleCriticalUpdateThrottled = throttle(() => this._scheduleCriticalUpdate(), 100, { trailing: true });
}
